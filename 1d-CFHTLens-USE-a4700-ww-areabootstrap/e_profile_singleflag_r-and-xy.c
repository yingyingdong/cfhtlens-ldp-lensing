...
int main(int argc,char **argv)
{
	...
	char file[1024]="/home/dfy/mw/dfy/CFHTLens/4wpz.tsv";
    ...
    for(nmag=1;nmag<2;nmag++)
    {
        ...
        for(zz=3;zz<4;zz++)
        {
            ...
            for(kk=0;kk<1;kk++)
            {
                ngroup = my_void_here_singleflag(group_pos,rcut[kk],zstep,zmeanvoid,nzbinvoid,mag_limit,mask_ratio
                        ,zmin,zsteplg,DAzlg);
...
#pragma omp parallel for private(l,i,rtmp)
                ...
                for(iboot=0;iboot<=nboot;iboot++)
                {
                    ...
                    sprintf(file,"data-2017/%s/w1-ww%lg-%s-zmean%lg-%d-%d-shear-cali2-%scra-%lg-%lg-%s-pz%lgomp-iboot%d-%d-%d-%d.dat",
                            dirout,ww,tmp2,zmeanvoid,ntr[0],ntr[1],tmp1,zbin_b[0][0],zbin_b[0][1],tmpwt,pz,iboot,griddec,gridra,nregion); 
                    printf("filename:%s\n",file); fflush(stdout);
                    if(flag_xy == 0)
