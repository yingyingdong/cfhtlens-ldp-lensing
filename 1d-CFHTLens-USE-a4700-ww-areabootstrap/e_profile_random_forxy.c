...
int main(int argc,char **argv)
{
	...
	FILE *fp;
    char dir[1024]="/home/dfy/data/CFHTLens",file[1024];
	//char file[1024]="/home/dfy/data/CFHTLens/4-jz-fields-pz.tsv";
	char tmp1[512],tmp2[512],tmp3[512],wtname[512],dirout[1024];
	char buf[1024],name[512],tmpwt[512]; 
    double magl[4]={-20.5,-21,-21.5,-22.},mag_limit,zmeanvoid=0.45; //zmeanvoid=0.512,0.45
	int i,j,l,nfit,flagc=-1,flagd=0,flag_lg=0,wt_sigma=0,flag_com=1,flag_xy=1,flag_density=0,nmag;
    if(flag_density == 0) sprintf(dirout,"onetime-pz");
    if(flag_density == 1) sprintf(dirout,"onetime-sigma-pz");
    int flag_all = 0;
    if(flag_all==0) sprintf(file,"%s/split-point/4w-goodpoint.dat",dir); //good pointings
    if(flag_all==1) sprintf(file,"%s/4wpz.tsv",dir); // all pointings
	...
	Lens = load_data_o(file,nobj,num,arr,&nfit,&zmax,xmax,ymax,zmeanvoid+0.1,flag_all);	
	...
	double (*group_pos)[6];
    group_pos = mymalloc(sizeof(double)*6*(20000000));
    ...
    for(nmag=1;nmag<2;nmag++)
    {
        ...
        for(zz=3;zz<4;zz++)
        {
            ...
            for(kk=0;kk<1;kk++)
            {
                ngroup = my_random_4field_groupflag(group_pos,rcut[kk],zstep,zmeanvoid,nzbinvoid,mag_limit,mask_ratio
                        ,zmin,zsteplg,DAzlg);
...
#pragma omp parallel for private(l,i,rtmp)
                for(l=0;l<nthread;l++)
                {
                    for(i=ngroup_assign[l];i<ngroup_assign[l+1];i++)
                        if(flag_group[nboot].flag[i]>0)
                        {
                            ...
                            if(flag_xy == 1) 
                                linklist_search_sub_pos2_pz_thread_xy(Lens,group_pos[i],tmp_shear[l],rtmp,rmin,hoc,ll,xrange,step,gstep,zbin_b[0],ntr,zsteplg,\
                ...
                for(iboot=0;iboot<=nboot;iboot++)
                {
                    ...
                    sprintf(file,"data-2017/%s/4field-ww%lg-%s-%d-%d-shear-cali2-%scra-%lg-%lg-%s-pz%lgomp-iboot%d-randomxy",
                            dirout,ww,tmp2,ntr[0],ntr[1],tmp1,zbin_b[0][0],zbin_b[0][1],tmpwt,pz,iboot); 
                    if(flag_all==0) sprintf(file,"%s-goodpoint.dat",file);
                    if(flag_all==1) sprintf(file,"%s.dat",file);
                    printf("filename:%s\n",file); fflush(stdout);
                    if(flag_xy == 0)
