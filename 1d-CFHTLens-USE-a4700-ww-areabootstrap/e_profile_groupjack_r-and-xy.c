...
int main(int argc,char **argv)
{
	...
	Lens = load_data_o(file,nobj,num,arr,&nfit,&zmax,xmax,ymax,zmeanvoid+0.1,1);	
    ...
    for(nmag=2;nmag<3;nmag++)
    {
        ...
        for(zz=3;zz<4;zz++)
        {
            ...
            for(kk=0;kk<1;kk++)
            {
                ...
                ngrp_use = ngroup;
                flag_group = flag_group_malloc_areajack(nboot,ngroup,nboot,group_pos);
...
#pragma omp parallel for private(l,i,rtmp)
                ...
                for(iboot=0;iboot<=nboot;iboot++)
                {
                    ...
                    sprintf(file,"data-2017/%s/4field-ww%lg-%s-%d-%d-shear-cali2-%scra-%lg-%lg-%s-pz%lgomp-ijack%d-group%d-%d-%d.dat",
                    //sprintf(file,"data-2017/%s/w1-ww%lg-%s-zmean%lg-%d-%d-shear-cali2-%scra-%lg-%lg-%s-pz%lgomp-iboot%d-group%d-%d-%d-%d.dat",
                            //dirout,ww,tmp2,zmeanvoid,ntr[0],ntr[1],tmp1,zbin_b[0][0],zbin_b[0][1],tmpwt,pz,iboot,griddec,gridra,stepdec,stepra); 
