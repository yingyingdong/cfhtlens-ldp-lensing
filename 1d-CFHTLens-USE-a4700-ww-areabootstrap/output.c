...
void gain_nr_gstep_o(int ntr[2],double gstep[2],int flag_lg,int flagd,int flag_xy,double *rmin,double *rmax)
{
    ...
    {
        ntr[0]=1; ntr[1]=60;
        if(flagd < 1) ntr[1] = 20; if(flag_lg==1) ntr[1]=40;//20;
...
{   
    ...
    for(i=0;i<ntr[0];i++)
        for(j=0;j<ntr[1];j++)
        {
            ...
            fprintf(fp,"%lg %lg %lg %lg %ld %lg %lg %lg %lg\n",
        ...
        double mag_limit,double zmean_v,double rcut,int nzbin_v,double zstep)
{
    ...
    if(flag_com == 1) sprintf(tmp1,"%s-COM",tmp);
...
int my_void_here_singleflag(double (*pos)[6],double rcut,double zstep,double zmean_v,int nzbin_v,
        double mag_limit,double mask_ratio,double zmin,double zsteplg,double *DAzlg)
{
    int nline,i,nz2; double tmp;
    FILE *fp;
    char dir[1024] = "../1d-CFHTLens-USE-a4700/data-2017/mask_flag/maskmethod1_USE/w1",file[1024];
    sprintf(file,"%s/w1_maskmethod1_nzbin%d-step0.006139-zstep%lg-zmean%lg-rmax%lg_magl%lg-maskratio%lg-pos-flag%d-%d-%d.dat",
            dir,nzbin_v,zstep,zmean_v,rcut/60.,mag_limit,mask_ratio,griddec,gridra,nregion);
    GET_line(file,&nline);
    myfopen(fp,file,"r");
    for(i=0;i<nline;i++)
    {
        ...
        double mag_limit,double mask_ratio,double zmin,double zsteplg,double *DAzlg)
{
    ...
    FILE *fp;
    char dir[1024] = "../1d-CFHTLens-USE-a4700/data-2017/mask_flag/maskmethod1_USE/total",file[1024];
    sprintf(file,"%s/4_maskmethod1_nzbin%d-step0.006139-zstep%lg-zmean%lg-rmax%lg_magl%lg-maskratio%lg-pos-flag-group%d-%d-%dfinal.dat",
            dir,nzbin_v,zstep,zmean_v,rcut/60.,mag_limit,mask_ratio,3000,stepdec,stepra);
    GET_line(file,&nline);
    myfopen(fp,file,"r");
    for(i=0;i<nline;i++)
    {
...
int my_void_here_groupflag(double (*pos)[6],double rcut,double zstep,double zmean_v,int nzbin_v,
        double mag_limit,double mask_ratio,double zmin,double zsteplg,double *DAzlg)
{   
    int nline,i,nz2; double tmp;
    FILE *fp;
    char dir[1024] = "../1d-CFHTLens-USE-a4700/data-2017/mask_flag/maskmethod1_USE/w1",file[1024];
    sprintf(file,"%s/w1_maskmethod1_nzbin%d-step0.006139-zstep%lg-zmean%lg-rmax%lg_magl%lg-maskratio%lg-pos-flag-group%d-%d-%d-%d.dat",
            dir,nzbin_v,zstep,zmean_v,rcut/60.,mag_limit,mask_ratio,griddec,gridra,stepdec,stepra);
    GET_line(file,&nline);
    myfopen(fp,file,"r");
    for(i=0;i<nline;i++)
    {   
        ...
        double mag_limit,double mask_ratio,double zmin,double zsteplg,double *DAzlg)
{
    int nline,i,nz2; double tmp;
    FILE *fp;
    char dir[1024] = "../1d-CFHTLens-USE-a4700/data-2017/mask_flag/4generate_random",file[1024];
    sprintf(file,"%s/4w-z%lg-magl%lgrandom.dat",dir,zmean_v,mag_limit);
    GET_line(file,&nline);
    myfopen(fp,file,"r");
    for(i=0;i<nline;i++)
    {
        fscanf(fp,"%lg %lg %lg",&pos[i][0],&pos[i][1],&pos[i][2]); //for void
        pos[i][0] = -pos[i][0];
        pos[i][2] = zmean_v;
        pos[i][5] = 0;
        nz2 = floor( (log10(1+pos[i][2])-log10(1+zmin))/zsteplg );
...
FLDP *map_ldp_field(int ngroup,int nfield,double (*pos)[6],int idf)
{
    int i,fid;
    FLDP *fldp;

    fldp = mymalloc(sizeof(FLDP)*nfield);
    for(i=0;i<nfield;i++)
    {
        fldp[i].n = 0;
        fldp[i].nid = 100;
        fldp[i].id = mycalloc(sizeof(int),fldp[i].nid);
    }
    for(i=0;i<ngroup;i++)
    {
        fid = (int)pos[i][idf];
        if(fldp[fid].n == fldp[fid].nid)
        {
            fldp[fid].nid *=2;
            fldp[fid].id = (int *)realloc(fldp[fid].id,sizeof(int)*fldp[fid].nid);
        }
        fldp[fid].id[fldp[fid].n] = i;
        fldp[fid].n ++;
    }
    for(i=0;i<nfield;i++) 
        fldp[i].id = (int *)realloc(fldp[i].id,sizeof(int)*fldp[i].n);
    return fldp;
}
...
FLAG_GROUP *flag_group_malloc_areaboot(int nnboot,int ngroup,double (*pos)[6])
{
    ...
    gsl_rng * r;
    int i,j,k,**rand,*field,nfield,idf=5,fid;
    FLAG_GROUP *flag_group;
    FLDP *fldp;

    nfield = 0;
    for(i=0;i<ngroup;i++)
        if(pos[i][idf]>nfield) nfield = (int)pos[i][idf];
    nfield = nfield+1;
    printf("nfield=%d\n",nfield);
    fldp = map_ldp_field(ngroup,nfield,pos,idf);
    ...
    for(j=0;j<nnboot;j++)
        ...
        {
            fid = rand[j][i];
            for(k=0;k<fldp[fid].n;k++)
                flag_group[j].flag[fldp[fid].id[k]] +=1;
        }
