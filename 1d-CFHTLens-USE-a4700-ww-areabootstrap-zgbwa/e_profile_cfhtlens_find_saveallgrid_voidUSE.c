...
void save_void_allpos(int **Nfound,int **flag_mask,int NDIV[2],int nzbin,double (*group_pos)[4],
...
int main(int argc,char **argv)
{
	...
	int i,j,nfit,flagd=0,nfield,mask_method=1,load_mask=1;
    double zbin[6],zmin=0.4,zstep=0.1,mag_limit=-20.5,xc[2],mask_ratio=1./10; int nzbin=1;	
...
#pragma omp parallel for private(i,rtmp)
            for(i=0;i<ngroup;i++)
            {
                ...
                Nfound[i] = linklist_search_sub_pos2_countzbin12(Lens,group_pos[i],rtmp,
                        hoc,ll,xrange,step,NDIV,nzbin,zstep,zmin,flag_mask,flagd,mask_ratio);
            }
            printf("end \n");
            ...
            sprintf(file,"data-2017/mask_flag/maskmethod1_USE/w%d/w%d_maskmethod%d_nzbin%d-step%lg-zstep%lg-zmin%lg-rmax%lg_magl%lg-maskratio%lg-nfound.dat",tmp,tmp,mask_method,nzbin,step[0],zstep,zmin,rmax[kk],mag_limit,mask_ratio);
            sprintf(tmp1,"data-2017/mask_flag/maskmethod1_USE/w%d/w%d_maskmethod%d_nzbin%d-step%lg-zstep%lg-zmin%lg-rmax%lg_magl%lg-maskratio%lg-nfoundpos.dat",tmp,tmp,mask_method,nzbin,step[0],zstep,zmin,rmax[kk],mag_limit,mask_ratio);
            ...
            save_void_allpos(Nfound,flag_mask,NDIV,nzbin,group_pos,tmp1,zstep,zmin,xc);
        }
    }
    free(Lens);
    return 0;
        ...
        for(i=0;i<NDIV[1];i++) //hang
        {
            for(j=0;j<NDIV[0];j++) //lie
            {
                ...
                if(flag_mask[i][j] == 1)
                {
                    ...
                    fprintf(fp,"%e %e %e %e %d\n",ra,group_pos[nelem][0],group_pos[nelem][1],
                            zmin+zstep*(k+0.5),Nfound[nelem][k]);
                    num++;//printf("there is!\n");
                }
            }
        }
