...
LENS *load_data(char *file ,int nobj,int num,double **arr,int *nfit,double *zmax,
        double xc[2],double mag_limit,double xrange[2][2]);
double **D_malloc2_here(int n,int NDIV[2]);
void good_data(double **arr,int nobj,int nfit,LENS *Lens,double *z,double xc[2],
        double mag_limit,double xrange[2][2]);
...
int main(int argc,char **argv)
{
	...
	int i,j,l,nfit,flagd=1,nfield=0,flag_lg=0,mask_method=1,flag_randp=0;
    double zbin[6],zmin,zmax,zmeanvoid=0.512,zstep;
    double mag_limit=-20.5,xc[2],mask_ratio=0.1; 
	...
	Lens = load_data(file,nobj,num,arr,&nfit,&zmax,xc,mag_limit,xrange);	
        ...
        for(kk=0;kk<5;kk++)  //rcut[kk]
        {
            ...
            for(i=0;i<ngroup;i++)
            {
                if(random[i]<=1./mul) 
                {
                    ...
                    {
                        Nfound[i] = linklist_search_sub_pos2_countzbin12_record_nomask(Lens,group_pos[i],rtmp,rmin,
                               gstep[1],hoc,ll,xrange,step,NDIV,nzbinvoid,zstep,zmeanvoid,
                               flagd,flag_lg,&flag_use,ntr);
                        flag_group[i]=1; 
                    }
                }
            ...
            int tmp=nfield+1;
            sprintf(file,"data-2017/number/w%d/w%d%smaskmethod%d_nzbin%d-step%lg-zstep%lg-zmean%lg-rcut%lg_magl%lg-maskratio%lg-%s-%s-mul%d-nfound_nomaskecut.dat",
		...
		double xc[2],double mag_limit,double xrange[2][2])
{
	...
	*nfit = tmp;
	...
	good_data(arr,nobj,tmp,Lens,zmax,xc,mag_limit,xrange);
	return Lens;
}
        ...
        double mag_limit,double xrange[2][2])
{
	...
	for(i=0;i<nobj;i++)
		...
		if(arr[i][11] > -98 && arr[i][11]<98 && arr[i][11]<mag_limit)
        {
			...
			if(xmax < Lens[tmp].pos[0]) xmax = Lens[tmp].pos[0];
			if(ymax < Lens[tmp].pos[1]) ymax = Lens[tmp].pos[1];
			if(zmax < Lens[tmp].pos[2]) zmax = Lens[tmp].pos[2];
			if(xmin > Lens[tmp].pos[0]) xmin = Lens[tmp].pos[0];
			if(ymin > Lens[tmp].pos[1]) ymin = Lens[tmp].pos[1];
						
			tmp ++;
		}
	printf("zmax = %f \n",zmax);
	*z = zmax;
    xrange[0][0] = xmin-0.1; xrange[0][1] = xmax+0.1;
    xrange[1][0] = ymin-0.1; xrange[1][1] = ymax+0.1;
}
