...
int main(int argc,char **argv)
{
	...
	int i,j,l,nfit,flagd=1,nfield=0,mask_method=1,load_mask=1,flag_lg=0,flag_randp=0;
    double zbin[6],zmin,zmax,zmeanvoid=0.35,zstep;
    double mag_limit=-21.,xc[2],mask_ratio=1./10; 
	...
	//search grid
	int pid,pindex,Nbound=0,kk,zz,nthread=50,*ngroup_assign;
    ...
    for(zz=3;zz<4;zz++)  //dz
    {
        ...
        for(kk=0;kk<2;kk++)  //rcut[kk]
        {
            ...
            if(flag_randp == 1)
            {
                ...
                //rand_point2d_mask(group_pos,ngroup,xrange,flag_mask,step,NDIV);
                for(i=0;i<ngroup;i++)
                {
