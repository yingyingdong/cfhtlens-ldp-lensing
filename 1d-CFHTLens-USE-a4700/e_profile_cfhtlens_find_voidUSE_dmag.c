...
double read_dmag(char *ww,double zmean,double Mag)
{
    ...
    sprintf(fname,"/home/dfy/work/2d-lensing/Mag/ww%s-%lg-%lgMag.dat",ww,zmean-0.1,zmean+0.1);
    myfopen(fp,fname,"r")
    fscanf(fp,"%lg %lg %lg",&dmag[0],&dmag[1],&dmag[2]);
    if(Mag==-21.)  return dmag[0];
    if(Mag==-21.5) return dmag[1];
    if(Mag==-22.)  return dmag[2];
}
...
int main(int argc,char **argv)
{
    ...
    double zbin[6],zmean=0.512,zstep,mag_limit,xc[2],mask_ratio=1./10; //zmean=0.512	
	...
	Lens = load_data(file,nobj,num,arr,&nfit,&zmax,xc,dmag);	
...
#pragma omp parallel for private(i,rtmp)
            ...
            sprintf(file,"data-2017/mask_flag/maskmethod1_USE/w%d/w%d_ww%s_maskmethod%d_nzbin%d-step%lg-zstep%lg-zmean%lg-rmax%lg_magl%lg-maskratio%lg-nfound.dat",tmp,tmp,ww,mask_method,nzbin,step[0],zstep,zmean,rmax[kk],mag_limit,mask_ratio);
            sprintf(tmp1,"data-2017/mask_flag/maskmethod1_USE/w%d/w%d_ww%s_maskmethod%d_nzbin%d-step%lg-zstep%lg-zmean%lg-rmax%lg_magl%lg-maskratio%lg-pos.dat",tmp,tmp,ww,mask_method,nzbin,step[0],zstep,zmean,rmax[kk],mag_limit,mask_ratio);
