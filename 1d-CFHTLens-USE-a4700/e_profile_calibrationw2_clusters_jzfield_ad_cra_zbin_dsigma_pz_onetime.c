...
void mean_shear(double cen[4],LENS *Lens,int Nfound,int *found,SHEAR **shear,
        ...
        double *DAzlg,double zmin,double zsteplg,int flagd,int flag_lg,int flag_wt,double pz,int flag_com);
...
void gain_fileo_name(int flag_lg,int flagd,int flagc,int wt_sigma,
        int flag_com,char *tmp1,char *tmp2,char *tmpwt,double mul);
...
int main(int argc,char **argv)
{
	...
	int i,j,nfit,flagc=-1,flagd=1,flag_lg=0,wt_sigma=1,flag_com=1;
	...
	double zmax,zmin=1e-3,xmax[2],ymax[2];
	LENS *Lens;
    ...
    else          {rmax=2000; rmin=50;}

	found = mymalloc(sizeof(int *)*ngroup);
	Nfound   = mymalloc(sizeof(int)*ngroup);
	memset(Nfound,0,sizeof(int)*ngroup);

    double rr = rmax,gstep[2],*random;
    int ntr[2] = {1,60};
    //int ntr[2] = {10,6};
    if(flagd < 1) ntr[1] = 20;
    
    //mul=(int)(ngroup/40000.); if(mul==0) mul=1; printf("ngroup=%d,mul=%lg",ngroup,mul);
    mul=(ngroup/40000.); if(mul<1) mul=1; printf("ngroup=%d,mul=%lg",ngroup,mul);
...
#pragma omp parallel for private(i,rtmp)
	for(i=0;i<ngroup;i++) 
	{
        ...
        //        group_pos[i][4]>=12.8 && group_pos[i][4]<13.5)
        {
...
//#pragma omp parallel for private(i,j)
	for(j=0;j<nzbin;j++)
	{
		...
		for(i=0;i<ngroup;i++)
		{
            if(Nfound[i] > 0)
            {
				mean_shear(group_pos[i],Lens,Nfound[i],found[i],shear[j],gstep,ntr,zbin_b[j],rmin,
    ...
    gain_fileo_name(flag_lg,flagd,flagc,wt_sigma,flag_com,tmp1,tmp2,tmpwt,mul);
    
    for(i=0;i<nzbin;i++)
	{
        //sprintf(file,"data-2017/onetime-sigma-pz/4jzfield-%s-%lg-%lg-%d-%d-shear-cali2-%scra-%lg-%lg-%s-pz%lg.dat",
		...
		shear_output(ntr,shear[i],shearr[i],file,gstep,rmin,flag_lg,nuse[i]);	
	}
        ...
        int flag_com,char *tmp1,char *tmp2,char *tmpwt,double mul)
{
    ...
    if(flag_com == 1) sprintf(tmp1,"%s-COM-mul%lg",tmp,mul);
    else sprintf(tmp,"%s-mul%lg",tmp,mul);
    ...
    //else if(flagc==-1) sprintf(tmp2,"void_mag-19_zmin0.2-rcut2.5ANG-maskratio0.1-nbin3-zstep0.1-step0.006139");
    //else if(flagc==-1) sprintf(tmp2,"void_mag-21_zmin0.2-rcut2.5ANG-nbin3-zstep0.1-step0.006139");
    //else if(flagc==-1) sprintf(tmp2,"void_mag-20_zmin0.4-rcut2.5-nbin1-zstep0.1-step0.006139");
    else if(flagc==-1) sprintf(tmp2,"void_mag-20.5_zmin0.4-rcut2-nbin1-zstep0.1-step0.006139");
    ...
    else if(flagc==-2) sprintf(tmp2,"cluster_randmul10");
    if(wt_sigma == 0) sprintf(tmpwt,"wt"); 
    else sprintf(tmpwt,"wtsigma"); 

    //nzbin3-step0.006139-zstep0.1-zmin0.2-rmax0.05COM_magl-20-maskratio0.01
        ...
        double *DAzlg,double zmin,double zsteplg,int flagd,int flag_lg,int flag_wt,double pz,int flag_com)
{
	...
	double dx,dy,dr,theta,tmp,alpha,sigmaz,a=1;
	...
	for(i=0;i<Nfound;i++)
	{
		...
		//if(POS(pid,2)<zbin[1] && POS(pid,2)>cen[2] && Lens[pid].pz>0 && Lens[pid].pz<pz)                     //change in 20150615
		{
			...
			if(wt>0 && dr>=rmin && grid[1] >= 0 && grid[1]<ntr[1])  
			{
                ...
                Lens[pid].angle = tmp;
				etmp[0] = et1(e1,e2,sin2a,cos2a)*wt*sigmaz/1e12;
				etmp[1] = et2(e1,e2,sin2a,cos2a)*wt*sigmaz/1e12;
				wt2 = wt*wt;
...
{   
	...
	for(i=0;i<ntr[0];i++)
		for(j=0;j<ntr[1];j++)
		{
			...
			tmp = gain_r_grid(flag_lg,gstep[1],rmin,j);
            fprintf(fp,"%f %f %f %f %d %f %f %f %f %e\n",
