...
void mean_shear(double cen[4],LENS *Lens,int Nfound,int *found,SHEAR **shear,
		double gstep[2],int ntr[2],double zbin[2],double rmin,int flagd,int flag_lg);
...
int main(int argc,char **argv)
{
	...
	int i,j,nfit,flagc=-10,flagd=0,flag_lg=0;
	...
	double rmax,rtmp,rmin;// /4./60; //10./60
	if(flagd < 1) {rmax=20./60;rmin=0.1/60;}
...
//#pragma omp parallel for private(i,j)
	for(j=0;j<nzbin;j++)
	{
		...
		for(i=0;i<ngroup;i++)
			...
			{
				if(i == 5000) printf("%d \n",i);
				mean_shear(group_pos[i],Lens,Nfound[i],found[i],shear[j],gstep,ntr,zbin_b[j],rmin,flagd,flag_lg);
			}
	}
    if(flag_lg == 0)
	...
	for(i=0;i<nzbin;i++)
	{
		...
		//sprintf(file,"data-2017/onetime/4jzfield-%s-%lg-%lg-%d-%d-shear-cali2-%scra-%lg-%lg.dat",
