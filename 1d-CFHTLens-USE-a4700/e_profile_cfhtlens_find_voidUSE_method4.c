...
LENS *load_data(char *file ,char *file_Mag,int nw,int nobj,int num,double **arr,int *nfit,double *zmax,
...
double read_dmag(char *ww,double zmean,double Mag)
{
    double k,b;
    FILE *fp;
    char fname[512];

    sprintf(fname,"data-2017/Method4_Mag_coef_for_obs%s-%lg-%lg.dat",ww,zmean-0.1,zmean+0.1);
    myfopen(fp,fname,"r");
    fscanf(fp,"%lg %lg",&k,&b);
    return k*Mag+b;
}
...
int gain_nw(char *ww)
{
    if (strcmp(ww, "ww-1") == 0) return 0;
    if (strcmp(ww, "ww-0.5") == 0) return 1;
    if (strcmp(ww, "ww-0.8") == 0) return 2;
    if (strcmp(ww, "ww-1.2") == 0) return 3;
    if (strcmp(ww, "ww-1zgb") == 0) return 4;
    if (strcmp(ww, "zgbwa") == 0) return 5;
}
...
int main(int argc,char **argv)
{
	...
	char dir[1024]="/home/dfy/mw/dfy/CFHTLens/";
	...
	char tmp1[512],tmp2[512],tmp3[512],buf[1024],name[512],file[1024],file_mag[1024];
    ...
    double **arr,zmax,xmax[2],ymax[2],xrange[2][2],dmag;
	//char ww[512]="ww-1zgb";
	char ww[512]="zgbwa"; 
	//char ww[512]="ww-1.2";
    int nw;
  
    nw = gain_nw(ww);  //which cosmology in 6 simu
    nfield = atoi(argv[1]); printf("nfield=%d\n",nfield); 
    mag_limit = atof(argv[2]);
    dmag = read_dmag(ww,zmean,mag_limit); printf("Mag=%lg,dmag=%lg",mag_limit,dmag); fflush(stdout);
    ...
    sprintf(file_mag,"%sw%d_cut_Mag6simu.dat",dir,nfield+1);
	get_line(file,&nobj);
	//nobj = 22724706;
	arr = D_malloc2(nobj,num+1);	
	Lens = load_data(file,file_mag,nw,nobj,num,arr,&nfit,&zmax,xc,dmag);	
...
#pragma omp parallel for private(i,rtmp)
            ...
            sprintf(file,"data-2017/mask_flag/maskmethod1_USE/w%d/method4_w%d_%s_maskmethod%d_nzbin%d-step%lg-zstep%lg-zmean%lg-rmax%lg_magl%lg-maskratio%lg-nfound.dat",tmp,tmp,ww,mask_method,nzbin,step[0],zstep,zmean,rmax[kk],mag_limit,mask_ratio);
            sprintf(tmp1,"data-2017/mask_flag/maskmethod1_USE/w%d/method4_w%d_%s_maskmethod%d_nzbin%d-step%lg-zstep%lg-zmean%lg-rmax%lg_magl%lg-maskratio%lg-pos.dat",tmp,tmp,ww,mask_method,nzbin,step[0],zstep,zmean,rmax[kk],mag_limit,mask_ratio);
		...
		double xc[2],double mag_limit)
{
	...
	{                                                                                                                                            
        ...
        for(i=0;i<6;i++) fscanf(fp_Mag,"%lf",&dtmp[i]);
        //if(n<100) printf("arr[%d], Mag=%lg,Mag0=%lg,Mag_nw=%lg\n",arr[n][11],dtmp[0],dtmp[1]);
        arr[n][11] = dtmp[nw];
        if(dtmp[0] > -98 && dtmp[0]<98 && dtmp[nw]<mag_limit)
            tmp ++;
		n ++;		                                                                  
	}                                                                  
	fclose(fp);                    
