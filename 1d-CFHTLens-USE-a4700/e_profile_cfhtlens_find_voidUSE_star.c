...
void gain_group_pos(int NDIV[3],double **xgrid,double (*group_pos)[4],int nfield,double xc[2]);
...
//when use the mask_method=0/1 to generate mask, comment out the part of selection for the catalogue in load_data and good_data
int main(int argc,char **argv)
{

	FILE *fp;
	char dir[1024]="/home/dfy/data/CFHTLens/star_flag/";
    ...
    int i,j,nfit,flagd=0,nfield=0,nzbin=1;
    //mask_method=1: convolve method toe generate mask, 0: zero pixels
    //load_mask  =1: load mask generated from above, 2: mask from venice
    int mask_method=1,load_mask=2;
    ...
    gain_group_pos(NDIV,xgrid,group_pos,nfield,xc);
    ...
    if(load_mask == 0)
        flag_mask = mask_region(NDIV[0],NDIV[1],hoc,ll,step,xgrid,xc[0],xc[1],mask_method,nfield);
    if(load_mask >= 1) 
        flag_mask = load_flag_mask(nfield,NDIV[0],NDIV[1],load_mask);
...
void gain_group_pos(int NDIV[3],double **xgrid,double (*group_pos)[4],int nfield,double xc[2])
{
    ...
    sprintf(fname,"data-2017/w%d_group_pos.dat",nfield+1);
    myfopen(fp,fname,"w");
    for(i=0;i<NDIV[1]*NDIV[0];i++)
    {
        fprintf(fp,"%lg %lg %lg\n",group_pos[i][0],group_pos[i][1],(group_pos[i][0]-xc[0])/cos(au*group_pos[i][1])+xc[0]);
    }
    fclose(fp);
}
