...
int clusters(double (*pos)[5],int num)
{
	...
	FILE *fp;
	char dir[512]="/home/fydong/work/clusters-caa/CFHTLens";
	char file[512];
	...
	for(i=0;i<num;i++)
	{
		...
		fscanf(fp,"%lf",&tmp2);
        pos[i][4] = log10(pow(10.,(0.161)*tmp2+12.39));
		fscanf(fp,"%d",&ntmp);
		printf("%lf %lf %lf %lf %lf %lf %d\n",pos[i][0],pos[i][1],pos[i][2],pos[i][3],pos[i][4],tmp2,ntmp);
	}
	fclose(fp);
...
int redmapper(double (*pos)[5],int num)
{
	...
	FILE *fp;
	char dir[512]="/home/fydong/work/clusters-caa/redmapper";
	char file[512];
...
int random_for_void(double (*pos)[5])
{
    ...
    for(i=0;i<nline;i++)
    {
        ...
        pos[tmp][3] = DH*qromb(angu_dia_d,0,pos[tmp][2])/(1.+pos[tmp][2]);
