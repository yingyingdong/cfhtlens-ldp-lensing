...
void save_void_pos(int **Nfound,int **flag_mask,int NDIV[2],
        int nzbin,double (*group_pos)[4],char *name,double zstep,double zmin);
...
int main(int argc,char **argv)
{
	...
	int i,j,nfit,flagd=0,nfield=3,mask_method=1,load_mask=0;
    double zbin[6],zmin=0.,zstep=0.1,mag_limit=-18.,xc[2]; int nzbin=5;	
    ...
    else flag_mask = load_flag_mask(nfield,NDIV[0],NDIV[1]);
		
	//search grid
	int **Nfound,pid,pindex,Nbound=0;
...
#pragma omp parallel for private(i,rtmp)
	for(i=0;i<ngroup;i++)
	{
		...
		Nfound[i] = linklist_search_sub_pos2_countzbin12(Lens,group_pos[i],rtmp,
				hoc,ll,xrange,step,NDIV,nzbin,zstep,zmin,flag_mask,flagd);
	}
	printf("end \n");
    ...
    sprintf(file,"data-2017/mask_flag/maskmethod1_USE/w%d/w%d_maskmethod%d_nzbin%d-step%lg-zstep%lg-zmin%lg-rmax%lg-nfound.dat",tmp,tmp,mask_method,nzbin,step[0],zstep,zmin,rmax);
    //sprintf(file,"data-2017/mask_flag/maskmethod1_USE/w%d/w%d_maskmethod%d_nzbin%d-step%lg-zstep%lg-rmax%lg-nfound.dat",tmp,tmp,mask_method,nzbin,step[0],zstep,rmax);
    //sprintf(tmp1,"data-2017/mask_flag/maskmethod1_USE/w%d/w%d_maskmethod%d_nzbin%d-step%lg-zstep%lg-rmax%lg-pos.dat",tmp,tmp,mask_method,nzbin,step[0],zstep,rmax);
    sprintf(tmp1,"data-2017/mask_flag/maskmethod1_USE/w%d/w%d_maskmethod%d_nzbin%d-step%lg-zstep%lg-zmin%lg-rmax%lg-pos.dat",tmp,tmp,mask_method,nzbin,step[0],zstep,zmin,rmax);
    save_found_zbin(Nfound,nzbin,NDIV[0],NDIV[1],file);
    save_void_pos(Nfound,flag_mask,NDIV,nzbin,group_pos,tmp1,zstep,zmin);
	return 0;
}
    ...
    for(k=0;k<nzbin;k++)
        for(i=0;i<NDIV[1];i++) //hang
        {
            for(j=0;j<NDIV[0];j++) //lie
            {
                ...
                {
                    fprintf(fp,"%e %e %e\n",group_pos[nelem][0],group_pos[nelem][1],zmin+zstep*(k+0.5));
                    num++;//printf("there is!\n");
                }
            }
        }
