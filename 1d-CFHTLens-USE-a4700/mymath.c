...
void rand_point2d_mask(double (*group_pos)[5],int ngroup,double xrange[2][2],int **mask,double step[2],int NDIV[2])
{
    int i,j,grid[2],ntmp=0;double *randx,*randy;


    randx = return_doublerand(2*ngroup,xrange[0][0],xrange[0][1]);
    randy = return_doublerand(2*ngroup,xrange[1][0],xrange[1][1]);

    for(i=0;i<2*ngroup;i++)
    {
        group_pos[ntmp][0] = randx[i];
        group_pos[ntmp][1] = randy[i];
        for(j=0;j<2;j++)
            grid[j] = floor((group_pos[ntmp][j]-xrange[j][0])/step[j]);
        if(grid[0]<NDIV[0] && grid[1]<NDIV[1])
            if(mask[grid[1]][grid[0]] == 1) ntmp++;
        if(ntmp == ngroup) break;
    }
    free(randx);
    free(randy);
}
