...
int main(int argc,char **argv)
{
    ...
    int i,j,nfit,flagd=0,nfield=0,mask_method=1,load_mask=1,nzbin=1,mul=8;
    double zbin[6],zmean=0.45,zstep,mag_limit=-21.5,xc[2],mask_ratio=1./10; //zmean=0.512	
	
	//------load data------//
	int nobj = 0,num=13;
	int NDIV[2],NDIV_mul[2],*grid_flag;
    ...
    step_mul[0] = step_mul[1] = 0.006139*mul;
    gain_NDIV_here(xrange,step,NDIV);
    gain_NDIV_here(xrange,step_mul,NDIV_mul);
    xgrid = gain_xgrid(NDIV_mul,xrange);
    ...
    ngroup = NDIV_mul[0]*NDIV_mul[1];
    group_pos = mymalloc(sizeof(double)*4*ngroup);
    gain_group_pos(NDIV_mul,xgrid,group_pos);
...
#pragma omp parallel for private(i,rtmp)
            ...
            sprintf(file,"data-2017/mask_flag/maskmethod1_USE/w%d/w%d_maskmethod%d_nzbin%d-step%lg-zstep%lg-zmean%lg-rmax%lg_magl%lg-maskratio%lg-mulgrid%dnfound.dat",tmp,tmp,mask_method,nzbin,step[0],zstep,zmean,rmax[kk],mag_limit,mask_ratio,mul);
            sprintf(tmp1,"data-2017/mask_flag/maskmethod1_USE/w%d/w%d_maskmethod%d_nzbin%d-step%lg-zstep%lg-zmean%lg-rmax%lg_magl%lg-maskratio%lg-mulgrid%dpos.dat",tmp,tmp,mask_method,nzbin,step[0],zstep,zmean,rmax[kk],mag_limit,mask_ratio,mul);
            ...
            save_found_zbin_o(Nfound,nzbin,NDIV_mul[0],NDIV_mul[1],file);
            save_void_pos_largegrid_o(Nfound,NDIV_mul,nzbin,group_pos,tmp1,zstep,zmean,xc);
        }
    }
    free(Lens);
    return 0;
