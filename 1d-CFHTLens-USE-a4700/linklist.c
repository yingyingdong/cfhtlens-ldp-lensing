...
int *linklist_search_sub_pos2_countzbin12(LENS *Lens,double cen[4],double rmax,int *hoc,int *ll,\
        ...
        double mask_ratio)
{
	int i,j,k,l,subbox_grid[2][2],cen_grid[2],nmax,len,flag = 0,ds=0;
...
#define POS(i,j) Lens[i].pos[j]
    for(i=0;i<2;i++)
    {
        ...
        cen_grid[i] = floor((cen[i]-range[i][0])/step[i]);
    }
    ...
    for(i=subbox_grid[0][0];i<subbox_grid[0][1]+1;i++)
    {
        for(j=subbox_grid[1][0];j<subbox_grid[1][1]+1;j++)
        {
            ...
            if(sqrt((cen_grid[0]-i)*(cen_grid[0]-i)*step[0]*step[0]+(cen_grid[1]-j)*(cen_grid[1]-j)*step[1]*step[1])<=rmax) 
            {
                ds++;
                if(flag_mask[j][i]==0) {flag ++;  continue;}
            }
