...
field,zm,magl,rmax = 1,'0.45','-21','0.0166667'
#grid
if(field == 1):
...
xr[:,0] = xr[:,0]-0.1
xr[:,1] = xr[:,1]+0.1
nx,ny = 128,128#256,256
ratio = 0.1
nii,njj = 32,32 #for y and x
grid = np.array([[nx,ny]])
...
#void
dirm = '/home/dfy/work/2d-lensing/1d-CFHTLens-USE-a4700/data-2017/mask_flag/maskmethod1_USE/w'+np.str(field)+'/'
ix,iy = 0,2
...
np.savetxt(dirm+'w'+np.str(field)+'_maskmethod1_nzbin1-step0.006139-zstep0.2-zmean'+zm+'-rmax'+rmax+'_magl'+magl+'-maskratio0.1-pos-flag-group'+np.str(Grid[field-1,0])+'-'+np.str(Grid[field-1,1])+'-'+np.str(nii)+'-'+np.str(njj)+'.dat',xvf)
