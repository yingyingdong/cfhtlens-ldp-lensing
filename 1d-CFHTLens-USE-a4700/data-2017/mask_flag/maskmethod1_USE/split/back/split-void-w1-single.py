...
for i in np.arange(grid[field-1,0]):
    for j in np.arange(grid[field-1,1]):
        ...
        if(nij>0) : 
            xvf[index1*index2,-1] = flag+np.arange(nij)
            flag = flag+nij
            #print nij
        nv = nv+nij
print nv
np.savetxt(dirm+'w'+np.str(field)+'_maskmethod1_nzbin1-step0.006139-zstep0.2-zmean'+zm+'-rmax'+rmax+'_magl'+magl+'-maskratio0.1-pos-flag'+np.str(grid[field-1,0])+'-'+np.str(grid[field-1,1])+'-'+np.str(nv)+'.dat',xvf)
