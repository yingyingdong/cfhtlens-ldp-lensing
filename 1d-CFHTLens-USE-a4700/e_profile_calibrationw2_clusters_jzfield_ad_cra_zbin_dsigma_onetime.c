...
int main(int argc,char **argv)
{
	...
	int i,j,nfit,flagc=1,flagd=1,flag_lg=1,flag_wt=1;

	
	
	//------load data------//
	int nobj = 0,num=13,nzbinlg=1000;
	...
	group_pos = mymalloc(sizeof(double)*4*ngroup_tmp);
	if(flagc == 0)
		...
		for(i=0;i<ngroup;i++)
			if(group_pos[i][2] >= zbin_c[0] && group_pos[i][2] < zbin_c[1])
			{
				...
				mean_shear(group_pos[i],Lens,Nfound[i],found[i],shear[j],gstep,ntr,zbin_b[j],rmin,
                        DAzlg,zmin,zsteplg,flagd,flag_lg,flag_wt);
                nuse[j]++;
			}
        printf("nuse[%d]=%d\n",j,nuse[j]);
	}
    if(flag_lg == 0)
    {if(flagd==0)  sprintf(tmp1,"r"); else sprintf(tmp1,"adr");}
    else {if(flagd==0)  sprintf(tmp1,"lgr"); else sprintf(tmp1,"lgadr");};
	if(flagc==0) sprintf(tmp2,"clusters");
	else if(flagc==1) sprintf(tmp2,"redmapper");
    //else  sprintf(tmp2,"galaxy-mag%lg",mag_limit);
	for(i=0;i<nzbin;i++)
	{
		if(flag_wt == 0)
        sprintf(file,"data-2017/onetime-sigma/4jzfield-%s-%lg-%lg-%d-%d-shear-cali2-%scra-zbin0.1-%lg-%lg.dat",
			tmp2,zbin_c[0],zbin_c[1],ntr[0],ntr[1],tmp1,zbin_b[i][0],zbin_b[i][1]); // 05 means : gird 0-5 
        else
            sprintf(file,"data-2017/onetime-sigma/4jzfield-%s-%lg-%lg-%d-%d-shear-cali2-%scra-zbin0.1-%lg-%lg-wtsigma.dat",
                    tmp2,zbin_c[0],zbin_c[1],ntr[0],ntr[1],tmp1,zbin_b[i][0],zbin_b[i][1]); 
		shear_output(ntr,shear[i],shearr[i],file,gstep,rmin,flag_lg);	
	}
        ...
        double *DAzlg,double zmin,double zsteplg,int flagd,int flag_lg,int flag_wt)
{
	...
	double dx,dy,dr,theta,tmp,alpha,sigmaz;
#define POS(i,j) Lens[i].pos[j]
	...
	for(i=0;i<Nfound;i++)
	{
		...
		//if(POS(pid,2)<zbin[1] && POS(pid,2)>cen[2])                     //change in 20150615
		{
			if(flagd==0)
			{
			...
			{
				sigmaz = sigma(cen[2],POS(pid,2),zmin,zsteplg,cen[3],DAzlg);
