...
int main(int argc,char **argv)
{
	...
	char file[1024]="/home/dfy/data/CFHTLens/4fields_pz.dat";
	char tmp1[512],tmp2[512],tmp3[512],wtname[512],dirout[1024];
	char buf[1024],name[512],tmpwt[512]; 
    double magl[7]={-20.5,-21,-21.5,-22.,-22.5,-98,-100},mag_limit,zmeanvoid=0.45; //zmeanvoid=0.512,0.45
	int i,j,l,nfit,flagc=0,flagd=1,flag_lg=1,wt_sigma=1,flag_com=1,flag_xy=0,flag_density=1,nmag;
    int flag_single = 0; //flag_single=1: divide magl<-21 into one bin
    if(flag_density == 0) sprintf(dirout,"onetime-pz");
    if(flag_density == 1) sprintf(dirout,"onetime-sigma-pz");
    //mag_limit = atoi(argv[1]);
	
	//------load data------//
	int nobj = 0,num=15,nzbinlg=1000,flag_read=0; //flag_read==1
	...
	Lens = load_data_o(file,nobj,num,arr,&nfit,&zmax,xmax,ymax,zmeanvoid+0.1,flag_read);	
	...
	double zbin_c[2]={0.6,1},pz=0.2;
	//double zbin_c[2]={0.4,0.7},pz=0.2;
    ...
    //double gstep[2],rcut[1]={1.2}; //arcmin
    ...
    int iboot,nzbinvoid=1,nn,**nuse,zz,ngroup_use,ngrp_use;
    nuse = I_calloc2(nthread,nboot+1);
    
    for(nmag=3;nmag<4;nmag++)
    {
        for(zz=3;zz<4;zz++)
        {
            zstep = 0.05*(1+zz);
            //ngroup = my_fore_galaxy_bino(group_pos,zstep,zmeanvoid,zmin,zsteplg,DAzlg,magl[nmag],magl[nmag+1]);
            if(flagc == 0)
            {    ngroup = clusters(group_pos,zbin_c); sprintf(name,"%s","cluster");}
            if(flagc == 1) 
            {    ngroup = redmapper(group_pos,zbin_c); sprintf(name,"%s","redmapper");}
            if(flagc == 2)
            {    sprintf(name,"%s","cmass_w4"); ngroup = cmass(group_pos,zbin_c,name);}
            //{    sprintf(name,"%s","cmass_4field"); ngroup = cmass(group_pos,zbin_c,name);}
            printf("ngroup=%d\n",ngroup);
            ...
            ngrp_use = ngroup;
            flag_group = flag_group_malloc_normalboot(nboot,ngroup,MIN_I(ngrp_use,ngroup));
...
#pragma omp parallel for private(l,i,rtmp)
            ...
            gain_file_name_fore(flag_xy,flag_lg,flagd,flagc,wt_sigma,flag_com,tmp1,tmp2,tmpwt,ngrp_use,zmeanvoid,nzbinvoid,zstep,magl[nmag],magl[nmag+1],name,flag_single);

            for(iboot=0;iboot<=nboot;iboot++)
            {
                add_shear_thread(nthread,shear,ntr,iboot);
                add_nuse_thread(nuse,nthread,iboot);
                sprintf(file,"data/%s/%s4field-ww%lg-%s-%d-%d-shear-cali2-%scra-%lg-%lg-%s-pz%lgomp-iboot%d.dat",
                        dirout,name,ww,tmp2,ntr[0],ntr[1],tmp1,zbin_c[0],zbin_c[1],tmpwt,pz,iboot); 
                printf("filename:%s\n",file); fflush(stdout);
                if(flag_xy == 0)
