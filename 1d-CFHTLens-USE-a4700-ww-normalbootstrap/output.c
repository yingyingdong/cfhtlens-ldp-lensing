...
void gain_file_name_fore(int flag_xy,int flag_lg,int flagd,int flagc,int wt_sigma,
        int flag_com,char *tmp1,char *tmp2,char *tmpwt,int ngrp_use,
        double zmean_v,int nzbin_v,double zstep,double mag1,double mag2,char *name,int flag_single)
{
    char tmp[512];
    ...
    if(flagc<10) sprintf(tmp2,"%s",name); 
    if(flagc==10) 
    {
...
int my_voidstartest_here_o(double (*pos)[5],double rcut,double zstep,double zmean_v,int nzbin_v,
        double mag_limit,double mask_ratio,double zmin,double zsteplg,double *DAzlg)
{
    int nline,i,nz2; double tmp;
    FILE *fp;
    char dir[1024] = "../1d-CFHTLens-USE-a4700/data-2017/mask_flag/maskmethod1_USE/total",file[1024];
    sprintf(file,"%s/4_maskmethod1_nzbin%d-step0.006139-zstep%lg-zmean%lg-rmax%lg_magl%lg-maskratio%lg-startest-pos.dat",
            dir,nzbin_v,zstep,zmean_v,rcut/60.,mag_limit,mask_ratio);
    GET_line(file,&nline);
    myfopen(fp,file,"r");
    for(i=0;i<nline;i++)
...
FLAG_GROUP *flag_group_malloc_normalboot(int nnboot,int ngroup,int nrand)
{
    const gsl_rng_type * T;
    gsl_rng * r;
    int i,j,**rand,*grpid,*index;
    FLAG_GROUP *flag_group;

    rand  = I_malloc2(nnboot,nrand);
    grpid = mymalloc(sizeof(int)*nrand);
    index = mymalloc(sizeof(int)*ngroup);
    flag_group = mymalloc(sizeof(FLAG_GROUP)*(nnboot+1));
    for(i=0;i<=nnboot;i++)
        flag_group[i].flag = mycalloc(sizeof(int),ngroup);
    for(i=0;i<ngroup;i++)
    ...
    for(i=0;i<nnboot;i++)
        gsl_ran_sample (r, rand[i], nrand, grpid, nrand, sizeof(int)); //sample with replacement
    gsl_rng_free (r);

    for(i=0;i<nnboot;i++)
        for(j=0;j<nrand;j++)
        {
            flag_group[i].flag[rand[i][j]] +=1;
            //flag_group[nnboot].flag[rand[i][j]] +=1;
        }
    for(i=0;i<nrand;i++) flag_group[nnboot].flag[grpid[i]] +=1;
    for(i=0;i<nnboot;i++) free(rand[i]); free(rand);
    free(grpid); free(index);
    return flag_group;
}
