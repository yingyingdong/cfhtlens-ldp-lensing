...
#pragma omp parallel for private(l,i,rtmp)
                ...
                for(iboot=0;iboot<=nboot;iboot++)
                {
                    ...
                    sprintf(file,"data-2017/%s/4jzfield-ww%lg-%s-zmean%lg-%d-%d-shear-cali2-%scra-%lg-%lg-%s-pz%lgomp-iboot%d-group%d-%d-%d.dat",
                            dirout,ww,tmp2,zmeanvoid,ntr[0],ntr[1],tmp1,zbin_b[0][0],zbin_b[0][1],tmpwt,pz,iboot,3000,stepdec,stepra); 
                    printf("filename:%s\n",file); fflush(stdout);
                    if(flag_xy == 0)
